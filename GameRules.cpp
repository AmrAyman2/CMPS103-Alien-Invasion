...
void GameRules::War() {
	...
	EarthArmy Earth;    
	AlienArmy Alien;   
	Generator generate(I1,Earth,Alien,*this);
	while (timeStep <= 40)
	{
...
void GameRules::test() {
	...
	for (int i = 0;i < 50;i++) {
		...
		if (A > 0 && A < 10) {
			if (human->ES_List.dequeue(earthtop)) {
				human->ES_List.enqueue(earthtop);
			}

		}
		else if (A > 10 && A < 20) {
			if (human->ET_List.pop(tanktop)) {
				getkilledlist(riphaydar);
				riphaydar.enqueue(tanktop);
			}
		}
		else if (A > 20 && A < 30) {
			if (human->EG_List.dequeue(topgun, pri)) {
			...
			for (int j = 0;j < 5;j++) {
				if (aliens->AS_List.dequeue(alientop)) {
					
					alientop->setHealth(*aliens->getHealth() - 1);
					templist.enqueue(alientop);
					templist.dequeue(alientop);
					aliens->AS_List.enqueue(alientop);
				}
			}
		}
		else if (A > 40 && A < 50) {
			for (int k = 0; k < 5;k++) {
				...
				if (aliens->AM_List.remove(monstertop)) {
			...
			for (int z = 0;z < 6;z++) {
				if (aliens->AD_List.dequeueRear(dronerear)) {
					killedlist.enqueue(dronerear);
				}
			}
			for (int m = 0;m < 6;m++) {
				if (aliens->AD_List.dequeue(dronetop)) {
